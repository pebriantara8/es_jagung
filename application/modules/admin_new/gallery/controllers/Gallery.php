...
class Gallery extends Grab {
	...
	public function __construct()
	{
		...
		//Do your magic here
        $this->posts = true;
        $this->path = './public/gallery/';
        $this->new_path = './public/gallery/thumb/';
	}
	...
	public function index()
	{
        ...
        $object['where'] = array('deleted_at IS NULL');
    ...
    public function archive()
    {
        ...
        $object['where'] = array('deleted_at IS NOT NULL');
	...
	public function form($id=null)
	{
        ...
        $data['title_content'] = "Form gallery";
        // $data['select2'] = true;
        // $data['jquery_confirm'] = true;
        $data['content'] = "form";
        $this->view($data,false);
    ...
    public function arsip($id=null,$go='')
    {
        ...
        $ob = array('deleted_at' => date('Y-m-d H:i:s'));
        $this->db->where($this->id_name, $id);
    ...
    public function purge_delete($id)
    {
        $dt = $this->wd_db->get_data_row('gallery', array('id'=>$id));
        $this->db->where('id', $id);
        $q=$this->db->delete('gallery');
        if($q){
            @unlink($this->path.$dt['image']);
            @unlink($this->new_path.$dt['image']);
            $this->session->set_flashdata('alert_success', 'Data berhasil dihapus');
            redirect(backend_url().$this->modul.'/archive');
        }else{
            $this->session->set_flashdata('alert_warning', 'Data gagal dihapus');
            redirect(backend_url().$this->modul);
        }
