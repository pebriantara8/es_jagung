...
class Dashboard extends Grab {
	...
	public function index()
	{
		...
		$data_post = $this->wd_db->get_data('posts');
		$data['total_post'] = count($data_post);
		
		$members = $this->wd_db->get_data('users');
		$data['total_member'] = count($members);

		$warta = $this->wd_db->get_data('warta');
		$data['total_warta'] = count($warta);

		// echo "string";
        $data['content'] = "index";
        $data['title_content'] = "Dashboard";
        $data['title_content_desc'] = "Panel dashboard GKi Adisucipto";
