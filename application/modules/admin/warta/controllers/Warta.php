...
class Warta extends Grab {
	...
	public function __construct()
	{
		...
		//Do your magic here
        $this->posts = true;
        $this->path = './public/warta/';
        // $this->new_path = './public/warta/thumb/';
	}
    ...
    public function save()
    {
        ...
        $upp = $this->main_model->upload_file_single($set);
        $nama_file = $upp['filename'];

        if($upp['error']){
            $this->session->set_flashdata('alert_error', 'Terjadi kesalahan sistem');
            $this->main_model->set_response_web('',$upp['error'],false);
        }else{
            $arr = array(
                'title' => $this->input->post('title'),
                'date' => date('Y-m-d H:i:s'),
                'file' => $upp['filename'],
                'user_id' => $this->session->userdata('id_user_admin'),
            );
            $q = $this->db->insert('warta', $arr);
            if($q){
                $this->session->set_flashdata('alert_success', 'Berhasil menyimpan warta');
                $this->main_model->set_response_web('','ok',true,200,backend_url().$this->modul);
            }else{
                @unlink('./public/warta/'.$upp['filename']);
                $this->session->set_flashdata('alert_error', 'Terjadi kesalahan sistem');
                $this->main_model->set_response_web('',$upp['error'],false);
            }
        }
    ...
    {
        $id = $this->input->post('id_warta');
        $dataw = $this->wd_db->get_data_row('warta', array('id_warta'=>$id));
        ...
        $this->db->where('id_warta', $id);
        $q = $this->db->update('warta', $arr);
        if($q){
            $this->session->set_flashdata('alert_success', 'Berhasil menyimpan warta');
            $this->main_model->set_response_web('','ok',true,200,backend_url().$this->modul);
        }else{
    ...
    public function delete_forever($id=null)
    {
        ...
        $dt = $this->wd_db->get_data_row('warta',array('id_warta'=>$id));
        $this->db->where('id_warta', $id);
        $del = $this->db->delete('warta');
        if ($del) {
            //delete file gambar utama
            @unlink($this->path.$dt['file']);
            @unlink($this->new_path.$dt['file']);
